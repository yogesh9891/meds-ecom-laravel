...
<body>
  ...
  <div>
    ...
    <table width='600px' border='0' cellspacing='0' cellpadding='0' bgcolor='#3f3fdd' style='font-family:Arial, Helvetica, sans-serif; font-size:13px;border-radius:8px;border-radius:8px;background: linear-gradient(90deg, rgba(138, 136, 136,1) 0%, rgb(254, 215, 195,1) 100%);border-radius:8px;border-radius:8px'><tr> <td style='padding:10px;'> 
...
<td style="width:50%;"><img  src="{{ asset('/img/911-Global-Meds-logo.webp') }}" style="width:75%;"> </td>

<td style="width:50%;" align="right"><a  style="font-size: 16px;color:white;" href="{{ url('/') }}"  >www.911GlobalMeds.com</a><p style="font-size: 14px;">911globalmeds.com 💊 Cares for You !</p></td></tr></tbody></table></td></tr>
...
<td style="color:black;font-size:13px;padding:5px 5px 5px 10px;">This is in reference to your <a href="{{route('order.invoice',$summary->c_14_7_order_invoice_url)}}" target="_blank" ><b>Order No. </b>{{$summary->c_14_2_online_invoice_no}}</a> dated  {{$summary->created_at->timezone('Asia/Kolkata')->format('d M, Y')}} for: <br aria-hidden="true">
...
<span >Your preference was to <strong> pay via Bank Transfer </strong>, and as such, <strong>please make a Bank Transfer / Online Transfer of 

 <span style="color: #3f3fdd">{{ $summary->c_14_94_forex_currency }} {{ round($summary->c_14_95_amount) }}* </span> into our bank account, details as mentioned on the <a href="{{route('order.invoice',$summary->c_14_7_order_invoice_url)}}" target="_blank" ><b>Invoice No. </b>{{$summary->c_14_2_online_invoice_no}}</a> </span></td></tr>
...
<td colspan="2" style="font-style:italic;">Due to Tax Exemption reasons,&nbsp;<span style="text-decoration:underline;"> no payment will be accepted from any Hong Kong or Canada Bank.</span><br aria-hidden="true">Such payment will be returned at the expense and cost of the sender.<br aria-hidden="true"><br>
                    ...
                    @if($summary->bank_account_address)
                      ...
                      <td style="padding:5px 5px 5px 5px;border-top: solid 0px #000;border-right: solid 1px #000;border-bottom: solid 1px #000;border-left: solid 1px #000;">Our Address</td>
...
<td style="padding:0px 10px;"><b>Note : Please send a copy of the Bank Transfer Receipt &amp; SWIFT Wire MT-103 Message sent by your bank when you have made the online / wire transfer.</b> <br aria-hidden="true"> <b>This will also be required to track your funds in our bank's system</b><br><br>
...
<td style="color:white;font-size:13px;border-radius:8px;border-radius:8px;background: linear-gradient(90deg, rgba(138, 136, 136,1) 0%, rgb(254, 215, 195,1) 100%);border-radius:8px;border-radius:8px;">
