...
<td style="color:black;font-size:13px;padding:5px 5px 5px 10px;">We thank you for your <a href="{{route('cart.invoice',[$summary->c_14_6_proforma_invoice_url??'url','p=1'])}}" target="_blank" ><b>Order No. </b>{{$summary->c_14_2_online_invoice_no}}</a> dated  {{$summary->created_at->timezone('Asia/Kolkata')->format('d M,Y')}} for :<br aria-hidden="true">
...
<p>As {{ $summary->c_14_29_receiver_country }} is not a general shipping destination for us, you are requested to contact the {{ $summary->c_14_29_receiver_country }}  DHL office to confirm if they will clear and deliver the parcel on the basis of your Doctor's Prescription / Scholarly Use Undertaking document.</p>
