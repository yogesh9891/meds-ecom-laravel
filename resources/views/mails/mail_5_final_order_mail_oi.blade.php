...
<td style="color:black;font-size:13px;padding:5px 5px 5px 10px;">With reference to your <a href="{{route('order.invoice',$summary->c_14_7_order_invoice_url)}}" target="_blank" ><b>Order No. </b>{{$summary->c_14_2_online_invoice_no}}</a> dated  {{$summary->created_at->timezone('Asia/Kolkata')->format('d M,Y')}}, we are pleased to inform that our Pharmacy has confirmed a ready stock of your ordered medications and has accepted your order for :<br aria-hidden="true">
...
<span >Please make a Bank Transfer / Online Transfer of  <span style="color: #3f3fdd">{{ $summary->c_14_94_forex_currency }} {{ round($summary->c_14_95_amount) }}*</span>  into our bank account, details as mentioned on the <a href="{{route('order.invoice',$summary->c_14_7_order_invoice_url)}}" target="_blank" ><b>Invoice No. </b>{{$summary->c_14_2_online_invoice_no}}</a> </span></td></tr>
...
<td style="padding:10px;">
            ...
            <p>As you wish to <strong>pay via Credit Card</strong>, you can pay <strong><span style="color: #3f3fdd">USD {{round($summary->c_14_84_total_amount)}}</span></strong>- by clicking on the Credit Card payment link below:</p>
             ...
             <strong> <p class="mb-5"><a class="wordbreak fw-600 text-underline blue-text" target="_blank"  href="{{ $summary->c_14_92_cc_request_link }}" target="_blank">{{ $summary->c_14_92_cc_request_link }}</a></p></strong>
              ...
              <p><strong>and so <span class="red-text">please fill ONLY YOUR CREDIT CARD DETAILS and NOTHING ELSE</span> in the above payment link.</strong></p>
