...
<body>
	...
	<div>
		...
		<table width='600px' border='0' cellspacing='0' cellpadding='0' bgcolor='#3f3fdd' style='font-family:Arial, Helvetica, sans-serif; font-size:13px;border-radius:8px;border-radius:8px;background: linear-gradient(90deg, rgba(138, 136, 136,1) 0%, rgb(255, 150, 138,1) 100%)'><tr> <td style='padding:10px;'> 
...
@php 
  ...
  if($summary->c_14_93_pay_mode == 'Bank'){
    $p =1;
      ...
      @endphp
<td style="color:black;font-size:13px;padding:5px 5px 5px 10px;">We thank you for your <a href="{{route('cart.invoice',[$summary->c_14_6_proforma_invoice_url??'url','p='.$p])}}" target="_blank" ><b>Order No. </b>{{$summary->c_14_2_online_invoice_no}}</a> dated  {{$summary->created_at->timezone('Asia/Kolkata')->format('d M,Y')}} for :<br aria-hidden="true">
...
<tr><td style="color:#6f6868;font-weight:bold;padding:5px 5px 5px 10px;">

<span >Please make a Bank Transfer / Online Transfer of <span style="color: #3f3fdd">{{ $summary->c_14_94_forex_currency }} {{ round($summary->c_14_95_amount) }}</span> into our bank account, details as mentioned on the <a href="{{route('cart.invoice',[$summary->c_14_6_proforma_invoice_url??'url','p='.$p])}}" target="_blank" ><b>Invoice No. </b>{{$summary->c_14_2_online_invoice_no}}</a>. </span></td></tr>
...
<td colspan="2" style="font-style:italic;">Due to Tax Exemption reasons,&nbsp;<span style="text-decoration:underline;"> no payment will be accepted from any Hong Kong or Canada Bank.</span><br aria-hidden="true">Such payment will be returned at the expense and cost of the sender.<br aria-hidden="true"><br>
                    ...
                    @if($bank_detail->c_5_3_account_name)
                    ...
                    <tr>
                      ...
                      <td style="padding:5px 5px 5px 5px;border-top: solid 0px #000;border-right: solid 1px #000;border-bottom: solid 1px #000;border-left: solid 0px #000;">{{$bank_detail->c_5_11_currency}}  {{ round($summary->c_14_84_total_amount) }}</td>
...
<td style="padding:0px 10px;"><br><b>Note : Please send a copy of the Bank Receipt &amp; SWIFT Wire MT-103 Message sent by your bank when you have made the online / wire transfer.</b> <br aria-hidden="true"> <b>This will also be required to track your funds in our bank's system</b><br><br>
...
<td style="padding:10px;">
	...
	<strong>Thank you for your interest in paying by Credit Card.</strong><br>.<br aria-hidden="true">

<strong>We will soon send you a CREDIT CARD PAYMENT LINK by WhatsApp/Email<font color="red">*</font> from <span style="color:blue;font-weight:bold;">WeCare@911GlobalMeds.com</span></strong> <br aria-hidden="true"> <br aria-hidden="true">

<font color="red">*</font>Due to our high mail volumes, please also check your email's SPAM folder, right-click on the mail and mark it as NOT SPAM.
...
<td style="color:white;font-size:13px;border-radius:8px;border-radius:8px;background: linear-gradient(90deg, rgba(138, 136, 136,1) 0%, rgb(255, 150, 138,1) 100%);">
